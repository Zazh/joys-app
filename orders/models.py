...
class Order(models.Model):
    ...
    def confirm_payment(self):
        ...
        # select_for_update на заказе и строках Stock.
        # ВАЖНО, чего `on_commit` НЕ делает: отправка не становится фоновой —
        # запрос (return-страница покупателя, callback банка) ждёт её как и
        # раньше. Он снимает блокировки ДО похода в SendPulse, и эти 55 секунд
        # перестают держать чужие оформления, которым нужен тот же Stock.
        # Бонус: если транзакция откатится, письмо об оплате не уйдёт.
