...
export function initCartModal() {
    ...
    cartOverlay.addEventListener('click', async (e) => {
        ...
        function applyCart(result, seq) {
            // Порядок проверок важен: обогнанный ответ выбрасывается молча и
            // loadCart() НЕ зовёт — иначе один устаревший ответ порождал бы
            // лишний GET и новую гонку (reqSeq, SB-03)
            if (seq !== reqSeq) return;
            if (!result || !result.ok) {
                // Сервер запрос не принял, а новое qty уже оптимистично в DOM —
                // перечитываем корзину, чтобы витрина не показывала число,
                // которого на сервере нет. Своего сообщения не заводим:
                // перечитанный список сам печатает состояние позиции
                loadCart();
                return;
            }
            cartData = result;
